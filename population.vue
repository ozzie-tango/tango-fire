...
<div>
  <div class="container">
    <label for="item[0].name">
      {{ item[0].header }}
      <input type="text" :id="item[0].name"
             v-model="item[0].input">
      <div class="error" v-if="item[0].input && isBadInput(item[0].name)">
        This field is required
      </div>
    </label>
    <label for="item[1].name">
      {{ item[1].header }}
      <input type="date" :id="item[1].name"
             v-model="item[1].input">
      <div class="error" v-if="item[1].input && isBadInput(item[1].name)">
        This field is required
      </div>
    </label>
    <label for="item[2].name">
      {{ item[2].header }}
      <input type="email" :id="item[2].name"
             v-model="item[2].input">
      <div class="error" v-if="item[2].input && isBadInput(item[2].name)">
        This field is required
      </div>
    </label>
    <label for="item[3].name">
      {{ item[3].header }}
      <input type="range" :id="item[3].name"
             v-model="item[3].input" min="0" max="87">
      <span>{{ item[3].input }}</span>
      <div class="error" v-if="item[3].input && isBadInput(item[3].name)">
        This field is required
      </div>
    </label>
  </div>

  <a v-if="item[3].input==87" class="welcome"
    @click="item[3].input=86">
    <p>
  ...
  <smart v-if="filteredRecords" :columns="input"
...
export default {
  ...
  computed: {
    ...
    newUser(){
      return {
        name: this.item[0].input,
        date: this.item[1].input,
        email: this.item[2].input,
        children: this.item[3].input
      }
    },
    /**
  ...
  methods: {
    ...
    isBadInput(a){
      ...
      switch(a){
      case 'name':
        output = this.item[0].input ? (this.item[0].input.length < 5)
                                       : true
        break
      case 'email':
        var regEmail = /^[\w\d!#$%&'*+-/=?^_`{|}~]+@[\w\d.]+\.[\w]{2,}$/
        output = this.item[2].input ? !this.item[2].input.match(regEmail)
                                       : true
      }
      return output
    }
